...
Chrome, Edge, and Safari check to see if _any_ side of the table cell has a border. If this is true for at least half of the cells in the table, those browsers expose the table as a data table.

Firefox does it slightly differently - it only checks the first table cell, and checks if it has a border on _all_ sides. If so, Firefox exposes the table as a data table.
